...
def predict():
    ...
    with open(sys.argv[2]) as f:
        model = js.load(f)
    # Extrait les dictionnaires des moyennes
    #   et des ecarts types de chaque matiere
    #     -> Ces chiffres serviront a standardiser
    #   les nouvelles notes du fichier de test comme pendant l'entrainement
    ...
    #     -> Ils mesurent l'importance de chaque matiere
    #           pour deviner la maison d'un eleve
    thetas = model["thetas"]
    ...
    for course_name in feature_means.keys():
        ...
        # Remplace dans la colonne de la matiere du DF de test
        #   les valeurs manquantes par la moyenne enregistree
        notes = df_test[course_name].fillna(mean)
        # Normalise toutes les notes de cette matiere
        #   et les stocke dans le nouveau dataframe de notes
        if std == 0:
            df_notes[course_name] = (notes - mean)
        else:
            df_notes[course_name] = (notes - mean) / std
    # Insere une premiere colonne nommee bias remplie de 1
    #   pour permettre le produit matriciel avec thetas_list
    df_notes.insert(0, "bias", 1)
    probabilities = {}
    # Parcourt les 4 modeles entraines
    # house: nom de la maison ;
    #   theta_list: biais et poids de chaque matiere pour une maison
    for house, theta_list in thetas.items():
        # Multiplie chaque note de l'élève
        #   par le poids de la matière correspondante, y ajoute le biais,
        #   puis additionne le tout
        #       pour obtenir son score global de compatibilité.
        z = np.dot(df_notes, theta_list)
        # Transforme le score de compatibilite
        #   en probabilite comprise entre 0 et 1
        #       et l'enregistre dans un dictionnaire
    ...
    # Cherche pour chaque ligne du DF de probabilites
    #   la probabilite la plus elevee,
    # extrait le nom de la colonne correspondante (la maison)
    #   et l'enregistre dans la colonne de la maison du DF de test
    df_test["Hogwarts House"] = df_probabilities.idxmax(axis=1)
    # Exporte les colonnes de l'index de l'eleve
    #   et de sa maison dans un nouveau fichier .csv
